First, we designed a simple method to  %SM: excellent methods section .. nice flow and terrific work Jesse! clearly you are thinking / thought of the same issues other experienced modelers did too :) but did a very nice job of explaining it
recover the generation time distribution
from parametric definitions of
the incubation period and the serial interval distributions.  %cite previous work here or better to include in discussion? what do you think?
Then, we applied this method to \covid using
distributions of XXX reported in the literature.   %distributions of?
...
serial interval distributions reported in the literature.  %match this with the objectives statement re: the 3 approaches compared?
% ==============================================================================
...
we used reported cases in the region between March 08 and April 15 2020	%SM: using date of report or date of symptom-onset (episode date)? [specify for reproducibility]
as the incidence time series $I(t)$.%
\footnote{We did not stratify incidence by
  ``imported'' vs ``local'' transmission events,
  in order to quantify overall epidemic growth.}  %SM: might want to give reason [i.e. would not influence our comparison - which was the objective here]
We smoothed $I(t)$ using a Gaussian kernel with $\sigma = 1$ day
to reflect uncertainty in reporting delay.
We then compared estimates of $\Re(t)$ using   %SM: Nice! would bring this up in objectives I think as well (see suggestions in intro section) 
